...
def venn3(labels, names=['A', 'B', 'C'], **options):
    ...
    figsize = options.get('figsize', (6, 6))
    dpi = options.get('dpi', 120)
...
def venn4(labels, names=['A', 'B', 'C', 'D'], **options):
    ...
    colors= ['blue','deepskyblue', 'red', 'black']
    ...
    # legend
    draw_text(fig, ax, 0.13, 0.18, names[0], 'black', fontsize=fontsize)
    draw_text(fig, ax, 0.18, 0.83, names[1], 'black', fontsize=fontsize)
    draw_text(fig, ax, 0.82, 0.83, names[2], 'black', fontsize=fontsize)
    draw_text(fig, ax, 0.87, 0.18, names[3], 'black', fontsize=fontsize)
    #leg = ax.legend(names, loc='best', fancybox=True)
    #leg.get_frame().set_alpha(0.5)
